...
RandSampled_Num = [100 100 100 100 100 100 100 100 100 100 100 100 100 100 100 100];
tr_lab = [];
tt_lab = [];
tr_dat = [];
...
%%   Calculate the sparse matix       
      sparsity_matrix = SOMP(tr_dat,tt_dat,60, 1,tr_lab );   
...
prob=reshape(prob,512*217,K);
prob=prob';
save prob_sa.mat prob
[a b]=max(prob);
result=reshape(b,512,217);
